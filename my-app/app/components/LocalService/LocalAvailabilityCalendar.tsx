...
const LocalAvailabilityCalendar: React.FC<LocalAvailabilityCalendarProps> = ({
  ...
  useEffect(() => {
    const fetchDateStatuses = async () => {
      ...
      const { data: localUserData, error: localUserError } = await supabase
        .from('local_user')
      ...
      if (localUserError) {
        console.error('Error fetching local_user data:', localUserError);
        return;
      }
      ...
      localUserData.forEach((item: { availability_id: number; status: string }) => {
